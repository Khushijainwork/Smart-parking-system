...
void ultrasonic_read(int pin_number) {
    /*
     * Reads the ultrasonic ranger and returns the value read to serial
     * in this format: U:{value}\n where value ranges from 0 to 1023
     */

    int distance = ultrasonic.MeasureInCentimeters(); // Reads the distance in cm
    Serial.print('U');
    Serial.print(pin_number);
    Serial.print(':');
    Serial.println(distance); // Adds a trailing \n
}
...
void servo_write(int pin_number, int position){
  /*
  * Writes the position to the servo at pin_number
  */
    ...
    if (Serial.available() > 0) {
        ...
        switch (operation){
            case 'R': // Read operation, e.g. RD12, RA4
				...
				} else if (mode == 'U'){ // Ultrasonic read
                    ultrasonic_read(pin_number);
                } else {
